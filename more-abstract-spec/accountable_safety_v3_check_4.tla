----------------------------- MODULE accountable_safety_v3_check_4 -----------------------------

(*  -   This specification is errornous because the initialization of C_j_primed.
    -   In this specification, while we check case 2, it is possible that C_j_primed and C_f_1
        have the same checkpoint slot. 
    -   This bug happens because of the missing constraint C_j_primed.chkp_slot /= C_f_1.chkp_slot
        in Init_C_j_2_and_C_j_primed.
*)
...
VARIABLES
    ...
    \* @type: Bool;
    exists_a_conflicting_justified_checkpoint_with_chkp_slot_of_C_f_1,   \*  If so, we check case 1. Otherwise, we check case 2.
    \* @type: Int;
    idx_of_B_j_primed,
    \* @type: $block;
    B_j_primed,                     \*  B' in Lemma 1.   
    \* @type: $checkpoint;
    C_j_primed, 
    \* @type: Set(Int);
    voters_of_C_f_1,                \*  Validators votes for the finalization or justification of C_f_1.    
    \* @type: Set(Int);
    voters_of_C_j_primed,           \*  Validators votes for the justification of B_j_primed.    
    \* @type: $checkpoint;
    C_j_2                           \*  C_j in Lemma 1. Assume C_j_2 in chain 2.           
...
(*  We require fork_idx <= LEN_1 - 1 and fork_idx <= LEN_2 - 1 to ensure that
        -   We can initialize two conflicting blocks B_f_1 and B_j_primed in chain_1
...
Init_C_f_1 ==
    ...
    /\  \E j_source \in set_of_checkpoints, j_target \in set_of_checkpoints : 
            LET ffg_vote == FFGVote(j_source, j_target)
            IN  IsFFGVoteForJustification(chain_1, LEN_1, C_f_1, ffg_vote)

(*  -   exists_a_conflicting_justified_checkpoint_with_chkp_slot_of_C_f_1 is deterministically initialized.
    -   If there exists no checkpoint that has the same checkpoint slot with C_f_1, case 1 cannot happen.
    -   If case 2 happens, there exits a justified checkpoint C_j_2 in chain_2 that conflicts with C_f_1
        and has the checkpoint slot greater than C_f_1's.
        
*)
InitCases ==
    /\  exists_a_conflicting_justified_checkpoint_with_chkp_slot_of_C_f_1 \in BOOLEAN 
    /\  ((  \A C \in set_of_checkpoints, idx \in 1..LEN_2 :
                    /\  fork_idx < idx
                    /\  chain_2[idx] = C.chkp_block
                    /\  C.chkp_slot /= C_f_1.chkp_slot 
            )
            => ~exists_a_conflicting_justified_checkpoint_with_chkp_slot_of_C_f_1
        )
    /\  (   exists_a_conflicting_justified_checkpoint_with_chkp_slot_of_C_f_1
            =>  (   \E c \in set_of_checkpoints, idx \in 1..LEN_2 :
                        /\  c.chkp_slot > C_f_1.chkp_slot
                        /\  fork_idx < idx
                        /\  c.chkp_block = chain_2[idx]
        ))
        

(*  C_j_2 is a justified block in chain_2 such that 
        - C_j_2 conflicts with C_f_1, and
        - C_f_1.chkp_slot <= C_j_2.chkp_slot.

    There exists an FFG vote to support the justification of C_j_2.
*)
Init_C_j_2 ==
    /\  C_j_2 \in set_of_checkpoints 
    /\  \E idx \in 1..LEN_2 : 
            /\ idx > fork_idx
            /\ chain_2[idx] = C_j_2.chkp_block
...
(*  Let C_j_primed be a checkpoint such that
        -   C_f_1.chkp_slot < C_j_primed.chkp_slot, and
        -   B_f_1 is not a prefix of B_j_primed.
    
    So, we know that
        -   B_j_primed is not in the postfix of chain_1.
        -   Either B_j_primed is a strict ancestor of B_f_1
            or B_j_primed conflicts with B_f_1.
        
    For every checkpoint c in set_of_checkpoints, we have
        -   c.chkp_slot >= C_j_primed.chkp_slot, 
        -   c.chkp_slot < C_f_1.chkp_slot
        -   c.chkp_slot < C_j_primed.chkp_slot and c.chkp_block is a descendant of C_f_1
    
    The block of C_f_1 is not a prefix of the block of C_j_primed.
    -   There exists a checkpoint C_target after C_j_primed such that C_target can become
        a target checkpoint of an FFG vote for the justification of C_j_primed. Hence,
        it implies C_target.chkp_slot = C_j_primed.chkp_slot.
*)
Init_C_j_primed_that_can_be_different_from_C_j_2 ==
    /\  C_j_primed \in set_of_checkpoints 
    /\  B_j_primed = C_j_primed.chkp_block
    /\  C_j_primed.chkp_slot > C_f_1.chkp_slot
    /\  \/  /\  idx_of_B_j_primed \in 1..LEN_2              (*  B_j_primed conflicts with B_f_1         *)
            /\  B_j_primed  = chain_2[idx_of_B_j_primed]
            /\  idx_of_B_j_primed > fork_idx
        \/  /\  idx_of_B_j_primed \in 1..LEN_1              (*  B_j_primed is a strict ancestor of B_f_1*)
            /\  B_j_primed  = chain_1[idx_of_B_j_primed]
            /\  idx_of_B_j_primed < idx_of_B_f_1
    /\  \A c \in set_of_checkpoints :
            ~IsAncestorCheckpoint(chain_1, LEN_1, C_f_1, c)
            =>  \/  c.chkp_slot <= C_f_1.chkp_slot
                \/  c.chkp_slot > C_j_primed.chkp_slot
                \/  /\  c.chkp_slot = C_j_primed.chkp_slot  (*  This constraint is missing in the current proof*)  
                    /\  \/  IsAncestorCheckpoint(chain_1, LEN_1, C_j_primed, c)
                        \/  IsAncestorCheckpoint(chain_2, LEN_2, C_j_primed, c)
    (*  ffg_vote is used in the justification of C_j_primed.
        Either the source checkpoint and C_f_1 have different checkpoint slots
        or the block of the source checkpoint is an ancestor of the block of C_f_1.
    *) 
    /\  \E  source, target \in set_of_checkpoints :
                LET ffg_vote == FFGVote(source, target)
                IN  /\  \/  source.chkp_slot /= C_f_1.chkp_slot
                        \/  IsAncestorCheckpoint(chain_1, LEN_1, source, C_f_1) 
                    /\  \/  IsFFGVoteForJustification(chain_2, LEN_2, C_j_primed, ffg_vote)
                        \/  IsFFGVoteForJustification(chain_1, LEN_1, C_j_primed, ffg_vote)
                            
\*  Set C_j_2 to C_j_primed
Init_C_j_primed_that_is_the_same_with_C_j_2 ==
    /\  C_j_primed = C_j_2
    /\  B_j_primed = C_j_primed.chkp_block
    /\  idx_of_B_j_primed \in 1..LEN_2              (*  B_j_primed conflicts with B_f_1         *)
    /\  fork_idx < idx_of_B_j_primed
    /\  B_j_primed = chain_2[idx_of_B_j_primed]   
        
(*  -   If case 1 happens, then we deterministcally pick a checkpoint C_j_2 that has the same 
        checkpoint slot with C_f_1. 
    -   Otherwise, then we deterministcally pick a checkpoint C_j_2 whose checkpoint slot 
        is greater than C_f_1's. 
    _   Because of InitCases, C_j_2 must exist. 
    -   Because of the existance of C_j_2, C_j_prime must exist.
*)
Init_C_j_2_and_C_j_primed ==
    \/  /\  exists_a_conflicting_justified_checkpoint_with_chkp_slot_of_C_f_1 
        /\  Init_C_j_2
        /\  C_f_1.chkp_slot = C_j_2.chkp_slot
        /\  Init_C_j_primed_that_is_the_same_with_C_j_2
...
(*  -   Deterministically pick two sets of validators that send messages to support
        the finalization or justification of C_f_1 and C_j_primed.
    -   The role of validators depends on the case that we are checking.
*)
InitVoters ==
    /\  voters_of_C_f_1 \in SUBSET Validators
    /\  3 * Cardinality(voters_of_C_f_1) >= 2 * N
    /\  voters_of_C_j_primed \in SUBSET Validators
    /\  3 * Cardinality(voters_of_C_j_primed) >= 2 * N  
...
(*  Case 1: C_f_1 and C_j_primed have the same checkpoint slot.
        -   vote_1 is an arbitrary vote that validator v can send to support the justification of C_f_1.
        -   vote_2 is an arbitrary vote that validator v can send to support the justification of C_j_primed.
        -   Note that vote_1 and vote_2 are over-approximated.
        -   We check that vote_1 and vote_2 are slashable.
    Because of Init's constraints, vote_1 and vote_2 must exist.
*)
Case1 ==
    LET voters == voters_of_C_f_1 \cap voters_of_C_j_primed
    IN  /\  3 * Cardinality(voters) >= N
        /\  \A v \in voters : \A s1, t1, s2, t2 \in set_of_checkpoints : 
                ...
                IN  /\  IsFFGVoteForJustification(chain_1, LEN_1, C_f_1, ffg_vote_1)
                    /\  IsFFGVoteForJustification(chain_2, LEN_2, C_j_primed, ffg_vote_2)
                    =>  IsSlashable(vote_1, vote_2)

(*  Case 2: C_f_1's checkpoint slot is less than C_j_primed's.
        -   vote_1 is an arbitrary vote that validator v can send to support the finalization of C_f_1.
        -   vote_2 is an arbitrary vote that validator v can send to support the justification of C_j_primed.
        -   Note that vote_1 and vote_2 are over-approximated.
        -   We check that vote_1 and vote_2 are slashable.
    Because of Init's constraints, vote_1 and vote_2 must exist.
    ...
    IN  /\  3 * Cardinality(voters) >= N
        /\  \A v \in voters : \A t1, s2, t2 \in set_of_checkpoints : 
                LET ffg_vote_1 == FFGVote(C_f_1, t1)
                    ...
                    vote_1 == Vote(v, ffg_vote_1)
                    vote_2 == Vote(v, ffg_vote_2)
                IN  (   \/  s2.chkp_slot /= C_f_1.chkp_slot
                        \/  IsAncestorCheckpoint(chain_1, LEN_1, s2, C_f_1)
                    )
                    =>  (   /\  /\  IsFFGVoteForFinalization(chain_1, LEN_1, C_f_1, ffg_vote_1)
                                /\  IsFFGVoteForJustification(chain_1, LEN_1, C_j_primed, ffg_vote_2)
                                =>  IsSlashable(vote_1, vote_2)
                            /\  /\  IsFFGVoteForFinalization(chain_1, LEN_1, C_f_1, ffg_vote_1)
                                /\  IsFFGVoteForJustification(chain_2, LEN_2, C_j_primed, ffg_vote_2)
                                =>  IsSlashable(vote_1, vote_2)
...
(*  Since the initialized state has two conflicting checkpoints C_f_1 and C_j_primed
    such that            
        -   C_f_1 is finalized and
        -   C_j_primed is justified,
    at least one third of validators is slashable.
*)
AtLeastOneThirdIsSlashable == 
    /\  exists_a_conflicting_justified_checkpoint_with_chkp_slot_of_C_f_1 
        =>  /\  C_f_1.chkp_slot = C_j_primed.chkp_slot 
            /\  Case1
    /\  ~exists_a_conflicting_justified_checkpoint_with_chkp_slot_of_C_f_1 
        =>  /\  C_f_1.chkp_slot < C_j_primed.chkp_slot 
            /\  Case2
