----------------------------- MODULE plausible_ffg -----------------------------


EXTENDS FiniteSets, Integers, typedefs, Apalache
...
(*  We require last_idx_in_shared_prefix <= LEN_1 - 1 and last_idx_in_shared_prefix <= LEN_2 - 1 to ensure that
 ...
 *)
InitFork ==
    /\  last_idx_in_shared_prefix \in 1..(LEN_1 - 2)
    /\  last_idx_in_shared_prefix <= LEN_2 - 1
    /\  \A i \in 1..LEN_1 : i <= last_idx_in_shared_prefix => chain_1[i] = chain_2[i]
    /\  \A i \in 1..LEN_1, j \in 1..LEN_2 : 
            /\  i > last_idx_in_shared_prefix
            /\  j > last_idx_in_shared_prefix
...
IsPlausibleFFGVoteForFinalization(chain, LEN, checkpoint, ffg_vote) ==
    /\  IsValidFFGVote(chain, LEN, ffg_vote)
    /\  ffg_vote.source = checkpoint
    /\  ffg_vote.target.chkp_slot = ffg_vote.source.chkp_slot + 1
           
\* @type: (Int -> $block, Int, $checkpoint, $ffgVote) => Bool;
IsPlausibleFFGVoteForJustification(chain, LEN, checkpoint, ffg_vote) ==
    /\  IsValidFFGVote(chain, LEN, ffg_vote)
...
Init_C_f_1 ==
    ...
    /\  idx_of_B_f_1 > last_idx_in_shared_prefix
    ...
    /\  \E f_target \in set_of_checkpoints : 
            ...
            IN  IsPlausibleFFGVoteForFinalization(chain_1, LEN_1, C_f_1, ffg_vote)
    /\  \E j_source, j_target \in set_of_checkpoints : 
            LET ffg_vote == FFGVote(j_source, j_target)
            IN  IsPlausibleFFGVoteForJustification(chain_1, LEN_1, C_f_1, ffg_vote)
...
Init_C_f_2 ==
    ...
    /\  \E idx \in 1..LEN_2 : 
            /\ idx > last_idx_in_shared_prefix
            /\ chain_2[idx] = C_f_2.chkp_block
    /\  \E f_target \in set_of_checkpoints : 
            LET ffg_vote == FFGVote(C_f_2, f_target)
            IN  IsPlausibleFFGVoteForFinalization(chain_2, LEN_2, C_f_2, ffg_vote)
    /\  \E j_source, j_target \in set_of_checkpoints : 
            LET ffg_vote == FFGVote(j_source, j_target)
            IN  IsPlausibleFFGVoteForJustification(chain_2, LEN_2, C_f_2, ffg_vote)
    /\  C_f_1.chkp_slot <= C_f_2.chkp_slot
 ...
 *)
Init_C_j_2_that_may_be_different_from_C_f_2 ==
    ...
    /\  \/  /\  idx_of_B_j_2 \in 1..LEN_2              (*  B_j_2 conflicts with B_f_1          *)
            ...
            /\  idx_of_B_j_2 > last_idx_in_shared_prefix
    ...
    /\  \E source, target, s1, t1 \in set_of_checkpoints : 
            ...
            IN  \/  /\  IsPlausibleFFGVoteForJustification(chain_2, LEN_2, C_j_2, ffg_vote)
                    /\  IsPlausibleFFGVoteForJustification(chain_2, LEN_2, source, ffg_vote_1)
                \/  /\  IsPlausibleFFGVoteForJustification(chain_1, LEN_1, C_j_2, ffg_vote)
                    /\  IsPlausibleFFGVoteForJustification(chain_1, LEN_1, source, ffg_vote_1)
...
AssumptionOnConflictingCheckpoint ==
    ...
    /\  \A c \in set_of_checkpoints :
            /\  c /= C_f_2
            /\  \E idx \in 1..LEN_2 : last_idx_in_shared_prefix < idx /\ c.chkp_block = chain_2[idx]
            =>  c.chkp_slot /= C_f_1.chkp_slot
...
Case1 ==
    ...
    IN  /\  3 * Cardinality(voters) >= N
        /\  \A v \in voters : \A s1, t1, s2, t2 \in set_of_checkpoints : 
                ...
                IN  /\  IsPlausibleFFGVoteForJustification(chain_1, LEN_1, C_f_1, ffg_vote_1)
                    /\  IsPlausibleFFGVoteForJustification(chain_2, LEN_2, C_j_2, ffg_vote_2)
                    =>  IsSlashable(vote_1, vote_2)
    ...
    IN  /\  3 * Cardinality(voters) >= N
        /\  \A v \in voters : \A t1, s2, t2 \in set_of_checkpoints : 
                LET ffg_vote_1 == FFGVote(C_f_1, t1)
                    ...
                    vote_1 == Vote(v, ffg_vote_1)
                    vote_2 == Vote(v, ffg_vote_2)
                IN  \/  /\  IsPlausibleFFGVoteForFinalization(chain_1, LEN_1, C_f_1, ffg_vote_1)
                        /\  IsPlausibleFFGVoteForJustification(chain_1, LEN_1, C_j_2, ffg_vote_2)
                        =>  /\ C_f_1.chkp_slot /= s2.chkp_slot 
                            /\ IsSlashable(vote_1, vote_2)
                    \/  /\  IsPlausibleFFGVoteForFinalization(chain_1, LEN_1, C_f_1, ffg_vote_1)
                        /\  IsPlausibleFFGVoteForJustification(chain_2, LEN_2, C_j_2, ffg_vote_2)
                        =>  /\ C_f_1.chkp_slot /= s2.chkp_slot 
                            /\ IsSlashable(vote_1, vote_2)
