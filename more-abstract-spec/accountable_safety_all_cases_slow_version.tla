...
VARIABLES
    ...
    set_of_checkpoints,
    \* @type: Set($checkpoint);
    set_of_justified_checkpoints,   \*  The introduction of set_of_justified_checkpoints makes the spec much challening to APALACHE.
    \* @type: Int;
    fork_idx,
    \* @type: Int;
    idx_of_B_f_1,
...
(*  -   set_of_justified_checkpoints is a subset of set_of_checkpoints.
        In our encoding, set_of_justified_checkpoints is an over-approximation of
        (actually) justified checkpoints. We do not encode the set of VOTE messages
        that are used in the justification of checkpoints in set_of_justified_checkpoints.
    -   Genesis_Block is in set_of_justified_checkpoints.
    -   There exits two conflicting checkpoints in set_of_justified_checkpoints.
 *)
InitJustifiedCheckpoints == 
    /\  set_of_justified_checkpoints \in SUBSET set_of_checkpoints
    /\  Genesis_Checkpoint \in set_of_justified_checkpoints
    /\  \E  C_1 \in set_of_justified_checkpoints, 
            C_2 \in set_of_justified_checkpoints, 
            idx1 \in 1..LEN_1, 
            idx2 \in 1..LEN_2 : 
                /\  fork_idx < idx1
                /\  C_1.chkp_block = chain_1[idx1]
                /\  fork_idx < idx2
                /\  C_2.chkp_block = chain_2[idx2]
...
Init_C_f_1 ==
    ...
    /\  C_f_1 \in set_of_justified_checkpoints
    ...
    /\  \E j_source \in set_of_justified_checkpoints, j_target \in set_of_checkpoints : 
            LET ffg_vote == FFGVote(j_source, j_target)
            IN  IsFFGVoteForJustification(chain_1, LEN_1, C_f_1, ffg_vote)
            
(*  C_j_2 is a finalized block in chain_2 such that 
        - C_j_2 conflicts with C_f_1, and
        - C_f_1.chkp_slot <= C_j_2.chkp_slot.
 *)
Init_C_j_2 ==
    /\  C_j_2 \in set_of_justified_checkpoints 
    /\  \E idx \in 1..LEN_2 : 
            /\ idx > fork_idx
            /\ chain_2[idx] = C_j_2.chkp_block
    /\  \E f_target \in set_of_checkpoints : 
            LET ffg_vote == FFGVote(C_j_2, f_target)
            IN  IsFFGVoteForFinalization(chain_2, LEN_2, C_j_2, ffg_vote)
...
Init_C_j_primed_that_can_be_different_from_C_j_2 ==
    /\  C_j_primed \in set_of_justified_checkpoints 
    ...
    /\  \A c \in set_of_justified_checkpoints :
     ...
     *) 
    /\  \E  source \in set_of_justified_checkpoints, 
            target \in set_of_checkpoints, 
            s1 \in set_of_justified_checkpoints, 
            t1 \in set_of_checkpoints : 
                LET ffg_vote == FFGVote(source, target)
                    ffg_vote_1 == FFGVote(s1, t1)
                IN  \/  /\  IsFFGVoteForJustification(chain_2, LEN_2, C_j_primed, ffg_vote)
...
InitCases ==
    IF \E C_1, C_2 \in set_of_justified_checkpoints, idx1 \in 1..LEN_1, idx2 \in 1..LEN_2 : 
            /\  C_1.chkp_slot = C_2.chkp_slot
            /\  fork_idx < idx1
            /\  fork_idx < idx2
            /\  C_1.chkp_block = chain_1[idx1]
            /\  C_2.chkp_block = chain_2[idx2]
...
\*  Case A: C_f_1 and C_j_primed have the same checkpoint slot.
CaseA ==
...
\*  Case A: C_f_1's checkpoint slot is less than C_j_primed's.
CaseB ==   
    LET voters == voters_of_C_f_1 \cap voters_of_C_j_primed
    IN  /\  3 * Cardinality(voters) >= N
...
AtLeastOneThirdIsSlashable ==
    /\  C_f_1.chkp_slot = C_j_primed.chkp_slot => CaseA
    /\  C_f_1.chkp_slot < C_j_primed.chkp_slot => CaseB
                    
Next == UNCHANGED <<    chain_1,
                        chain_2,
                        set_of_checkpoints,
                        set_of_justified_checkpoints,
                        fork_idx,
                        idx_of_B_f_1,
                        B_f_1,
                        C_f_1,
