...
\item 
 ...
 Otherwise, it remains to be shown that
\[
G(x, F(h(f[x]), \mop(f'))) = \op(x)
\]
For the final case split, we look at whether $P(x)$ holds or not. If it does not, then 
\[
\op(x) = G(x, F(h(V(x)), Op))
\]
...
\]
By the property of $f$, $h(f[x]) = h(V(x))$. As $x \in D_f$, $h(f[x]) \subseteq D_{f'}$, so for every element $v \in h(V(x))$ it is the case that $Op(v) = \mop(f')[v]$. Since $F$ is either a map or a filter, the equality above follows.

The last case is when $P(x)$ holds, but $x \in D_{\mop(f')} \setminus D_f$. We know that $\op(x) = e$, and by Lemma \ref{lemma1}, $h(f[x]) = \emptyset$.
Then, 
