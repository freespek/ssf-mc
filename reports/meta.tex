...
\end{lstlisting}
%
Here, $\ApaFoldSeqLeft$ is the left-fold over sequences, that is, an operator for which
\begin{align*}
...
\]
%
We can see that $\Chain(x,N)$ returns the sequence $\tup{v_n, ..., x}$ if $N$ is sufficiently large. 
